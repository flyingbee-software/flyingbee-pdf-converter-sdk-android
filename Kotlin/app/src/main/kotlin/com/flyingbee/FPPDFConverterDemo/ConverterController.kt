...
import com.flyingbee.FPPDFFramework.Kotlin.ConversionFormat
import com.flyingbee.FPPDFFramework.Kotlin.ConversionOptions
import com.flyingbee.FPPDFFramework.Kotlin.ConversionRequest
import com.flyingbee.FPPDFFramework.Kotlin.ConversionResult
import com.flyingbee.FPPDFFramework.Kotlin.FPPDFConverter
import com.flyingbee.FPPDFFramework.Kotlin.FPPDFDocument
import com.flyingbee.FPPDFFramework.Kotlin.FPPDFFramework
import com.flyingbee.FPPDFFramework.Kotlin.ImageFormat as SdkImageFormat
import com.flyingbee.FPPDFFramework.Kotlin.OutlineType
import com.flyingbee.FPPDFFramework.Kotlin.PageRange
...
//
// Like the Java demo's multi-Activity navigation, every screen is its own
// Activity, so the controller can no longer live in a single Activity's
// ViewModelStore. It is instead a process-wide singleton owned by the
// Application context (see DemoApplication); each Activity reads and writes the
// same instance. State stays in Compose mutableStateOf so the foreground
// Activity's composition recomposes on change.
// ---------------------------------------------------------------------------

class ConverterController private constructor(app: Application) {
    ...
    fun startup() {
        // The singleton survives the Activity recreation triggered by an
    ...
    /** Releases the native SDK resources. The process-wide singleton lives as
     *  long as the Application, so this is only called if the app is torn down;
     *  a running conversion holds the native exporter, so cancel first and let
     *  the job settle before the scope is torn down. */
    fun shutdown() {
        converter?.cancel()
    ...
    private fun buildConversionRequest(source: File): ConversionRequest {
        ...
        val options = ConversionOptions().apply {
            ...
            html {
                layoutMode = com.flyingbee.FPPDFFramework.Kotlin.HtmlLayoutMode.entries.getOrElse(htmlLayoutMode) { com.flyingbee.FPPDFFramework.Kotlin.HtmlLayoutMode.EXACT_PAGE }
                mergeResource = com.flyingbee.FPPDFFramework.Kotlin.HtmlMergeResource.entries.getOrElse(htmlMergeResource) { com.flyingbee.FPPDFFramework.Kotlin.HtmlMergeResource.CSS_JS_SMALL_IMAGES }
                navigationBar = com.flyingbee.FPPDFFramework.Kotlin.HtmlNavigationBar.entries.getOrElse(htmlNavigationBar) { com.flyingbee.FPPDFFramework.Kotlin.HtmlNavigationBar.PDF_VIEWER }
                textFlowParagraph = com.flyingbee.FPPDFFramework.Kotlin.HtmlTextFlowParagraph.entries.getOrElse(htmlTextFlowParagraph) { com.flyingbee.FPPDFFramework.Kotlin.HtmlTextFlowParagraph.LINE_BREAK }
                packageAsZip = htmlPackageZip
            }
            excel {
                formatOption = com.flyingbee.FPPDFFramework.Kotlin.ExcelFormatOption.entries.getOrElse(excelFormatOption) { com.flyingbee.FPPDFFramework.Kotlin.ExcelFormatOption.KEEP_ORIGINAL_FORMATTING }
                thousandSeparator = com.flyingbee.FPPDFFramework.Kotlin.ExcelThousandSeparator.entries.getOrElse(excelThousandSeparator) { com.flyingbee.FPPDFFramework.Kotlin.ExcelThousandSeparator.AUTO }
                allInOneSheet = excelAllInOneSheet
                allInOneSheetAddToRow = excelAllInOneStyle == 0
                recognizeNumber = excelRecognizeNumber
                overlapText = com.flyingbee.FPPDFFramework.Kotlin.ExcelOverlapText.entries.getOrElse(excelOverlapText) { com.flyingbee.FPPDFFramework.Kotlin.ExcelOverlapText.AUTO }
            ...
            ocr {
                ...
                engineMode = com.flyingbee.FPPDFFramework.Kotlin.OcrEngineMode.LSTM_ONLY
                resizeDPI = ocrDPI
                minConfidence = 10.0f
    ...
    companion object {
        @Volatile
        private var instance: ConverterController? = null

        /** Created once by [DemoApplication.onCreate]; every Activity reads the
         *  same instance, mirroring the Java demo's ConverterController.get(). */
        fun init(app: Application) {
            if (instance == null) {
                synchronized(this) {
                    if (instance == null) instance = ConverterController(app)
                }
            }
        }

        val shared: ConverterController
            get() = instance ?: throw IllegalStateException("Call init(app) first")
    }
}
