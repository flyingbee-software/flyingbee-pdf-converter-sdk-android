...
Copy `SDK/libs/flyingbee/FPPDFFramework-10.3.6.aar` (the repo-root shared copy) into your own app (e.g. `app/libs/`), then declare it as a file dependency:
...
Copy the **entire `Resources.bundle/` folder** from the SDK drop (or from this repository's [`SDK/assets/Resources.bundle/`](SDK/assets/)) into your app at `app/src/main/assets/Resources.bundle/`. This is required because the AAR no longer carries the runtime resources.
